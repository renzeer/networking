...
void print_answer (char * buffer, int num_bytes) {
    // bytes 30 & 31 = type
}
...
int main (int argc, char ** argv) {
    ...
    char type_buffer[5];
    snprintf(type_buffer, sizeof(type_buffer), "%02x%02x", recv_buffer[rcvd-26], recv_buffer[rcvd-25]);


    printf("%s:\n", argv[1]);
    if (strcmp("001c\0",type_buffer) == 0) {
        char ip6[INET6_ADDRSTRLEN];
        inet_ntop(AF_INET6, recv_buffer+(rcvd-16), ip6, INET6_ADDRSTRLEN);
        printf("AAAA %s\n", ip6);
    } else {
        printf("AAAA <none>\n");
    }
